...
- (void)setRequestDelegate:(id)value {
	requestDelegate = value;
}
...
- (void)handleNewConnectionFromAddress:(NSData *)addr inputStream:(NSInputStream *)istr outputStream:(NSOutputStream *)ostr {
    HTTPConnection *connection = [[connClass alloc] initWithPeerAddress: addr
															inputStream: istr
														   outputStream: ostr
															  forServer: self];
...
- (void)performDefaultRequestHandling:(HTTPServerRequest *)mess {
    ...
    if ([method isEqual:@"GET"] || [method isEqual:@"HEAD"]) {
		...
		NSDictionary* result = [[[self server] requestDelegate] performSelector: @selector(handleGetRequestWithURI:URL:)
																	 withObject: uri
																	 withObject: url];
		NSData* data; NSString* MIMEType = nil;
        if (!result) {
            CFHTTPMessageRef response = CFHTTPMessageCreateResponse(kCFAllocatorDefault, 404, NULL, kCFHTTPVersion1_1); // Not Found
			data = [@"Not found" dataUsingEncoding: NSASCIIStringEncoding];
			...
			MIMEType = [result objectForKey: @"MIMEType"];
			data = [result objectForKey: @"Data"];
			//NSLog(@"# %@ %u", MIMEType, [data length]);
		}

        CFHTTPMessageRef response = CFHTTPMessageCreateResponse(kCFAllocatorDefault, 200, NULL, kCFHTTPVersion1_1); // OK
		if (MIMEType != nil) CFHTTPMessageSetHeaderFieldValue(response, (CFStringRef)@"Content-Type", (CFStringRef)MIMEType);
        CFHTTPMessageSetHeaderFieldValue(response, (CFStringRef)@"Content-Length", (CFStringRef)[NSString stringWithFormat:@"%d", [data length]]);
        if ([method isEqual:@"GET"]) {
            CFHTTPMessageSetBody(response, (CFDataRef)data);
        }
	...
	NSData* data = [@"Method not supported" dataUsingEncoding: NSASCIIStringEncoding];
	CFHTTPMessageSetHeaderFieldValue(response, (CFStringRef)@"Content-Length", (CFStringRef)[NSString stringWithFormat:@"%d", [data length]]);
		CFHTTPMessageSetBody(response, (CFDataRef)data);
    [mess setResponse:response];
    CFRelease(response);
